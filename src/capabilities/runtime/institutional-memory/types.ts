...
export interface TemporalDensityMap {
  period: string;
  anomalyDensity: number;
  recurrenceDensity: number;
  fatigueDensity: number;
  treasuryInstabilityDensity: number;
  operationalPressureDensity: number;
  governanceBreakdownDensity: number;
}
...
export interface TemporalCausalityOutput {
  ...
  // Temporal Series for Visualizations (Dummy Renderers)
  trajectorySeries?: TemporalTrajectoryPoint[];
