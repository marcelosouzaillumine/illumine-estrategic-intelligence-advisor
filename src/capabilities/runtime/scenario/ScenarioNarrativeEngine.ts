...
export class ScenarioNarrativeEngine {
  /**
   * Gera o Advisory Preditivo (linguagem executiva determinística) do contexto.
  ...
  static generateNarrative(shocks: ScenarioShock[], propagation: ScenarioPropagationResult, finalConfidence: ProjectedConfidence): ScenarioNarrative {
    ...
    let summary = `Sob um contexto submetido a ${shocks.length} vetores de choque institucional, `;
    ...
    if (stress.collapsedEntities.length > 0) {
      ...
      if (stress.groupSolvencyStatus === 'SOLVENT') {
        summary += `a estrutura corporativa mantém solvência, demonstrando resiliência aos impactos de curto e médio ciclo. `;
      } else {
