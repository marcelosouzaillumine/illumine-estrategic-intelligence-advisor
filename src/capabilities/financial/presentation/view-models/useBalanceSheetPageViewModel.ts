...
export function useBalanceSheetPageViewModel({ clients, selectedClient, selectedYear }: any) {
  ...
  // diretamente (ver mapToFinancialPositionPureViewModel), mas alimenta-o.
  const canonicalViewModel = useMemo(() => {
    if (!hasBalanceSheetData || !bpSummary) return null;
    return BalanceSheetExecutiveViewModelBuilder.build(
      executiveReport || {},
      'safe',
      filterYear,
      bpSummary,
  ...
  }, [hasBalanceSheetData, bpSummary, canonicalIndicators, filterYear, executiveReport]);
