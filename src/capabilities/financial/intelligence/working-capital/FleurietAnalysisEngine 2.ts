import { FleurietAnalysisResult, FleurietClassificationType, FleurietRiskLevel } from '../../domain/types/financial-domain.types';
...
export class FleurietAnalysisEngine {
  static analyze(cgl: number, ncg: number, treasury: number): FleurietAnalysisResult {
    ...
    if (isCglPositive && !isNcgPositive && isTreasuryPositive) {
      ...
      riskLevel = 'MEDIUM';
      description = 'Empresa possui capital de giro próprio, mas utiliza recursos de ciclo imediato para financiar a necessidade operacional. Saldo de tesouraria negativo.';
    } else if (!isCglPositive && isNcgPositive && !isTreasuryPositive) {
      type = 'TYPE_4';
