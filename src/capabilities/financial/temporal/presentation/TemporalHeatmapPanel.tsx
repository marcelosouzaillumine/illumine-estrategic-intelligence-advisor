...
export const TemporalHeatmapPanel: React.FC<TemporalHeatmapPanelProps> = ({ densityMaps }) => {
  ...
  return (
    <div className="temporal-heatmap-panel p-6 bg-slate-900 border border-border rounded-lg shadow-md text-muted-foreground overflow-x-auto">
      <div className="mb-6 border-b border-border pb-4 min-w-[600px]">
        <h3 className="text-lg font-semibold text-muted-foreground">Temporal Event Density Heatmap</h3>
        <p className="text-xs text-muted-foreground mt-1">Cross-sectional density of institutional stress markers over time</p>
      </div>

      <div className="min-w-[600px]">
        <div className="grid grid-cols-[200px_repeat(auto-fit,minmax(50px,1fr))] gap-1 mb-2">
          <div className="text-xs font-semibold text-muted-foreground uppercase">Marker</div>
          {densityMaps.map((map, idx) => (
            <div key={idx} className="text-center text-xs font-mono text-muted-foreground">{map.period}</div>
          ))}
        </div>

        {keys.map(({ key, label }) => (
          <div key={key} className="grid grid-cols-[200px_repeat(auto-fit,minmax(50px,1fr))] gap-1 mb-1 items-center">
            <div className="text-sm text-muted-foreground truncate pr-2">{label}</div>
            {densityMaps.map((map, idx) => {
