...
import { useExecutiveAdvisoryEngine } from '../engine/ExecutiveAdvisoryEngine';
import { MessageTimeline } from './MessageTimeline';

export function ExecutiveAdvisoryWidget() {
  const [isOpen, setIsOpen] = useState(false);
  const [isVisible, setIsVisible] = useState(false);
  const { t } = useTranslation('advisory');
  const engine = useExecutiveAdvisoryEngine(t);
      ...
      >
        ...
        <div className="p-4 border-b border-white/5 bg-[#121214] flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            {showBackButton ? (
              <button onClick={handleReset} className="text-slate-500 hover:text-white transition-colors p-1" title={t('widget.reset', 'Reiniciar conversa')}>
                <RotateCcw size={16} />
              </button>
            ) : (
