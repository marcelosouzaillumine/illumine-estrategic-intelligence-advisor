  ...
  if (!active) {
    return (
      <div className="flex justify-end p-4">
        <button
          onClick={onToggle}
          className="bg-slate-900 border border-border text-muted-foreground px-4 py-2 rounded text-xs font-mono hover:bg-slate-800 hover:text-white"
        >
  ...
  return (
    <div className="fixed inset-0 bg-slate-950 z-50 overflow-y-auto flex flex-col p-8 space-y-6">
      <div className="flex items-center justify-between border-b border-border pb-4">
        ...
        <button
          onClick={onToggle}
          className="bg-slate-900 border border-border text-red-400 px-3 py-1.5 rounded text-xs font-mono hover:bg-red-950/20"
        >
          Exit Presentation
        </button>
      </div>
