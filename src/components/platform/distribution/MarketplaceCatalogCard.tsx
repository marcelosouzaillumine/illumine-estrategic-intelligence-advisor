  ...
  return (
    <ExecutiveSurface className="p-4 mb-4 bg-card border border-border rounded-lg shadow-sm">
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground mt-2">
        <span>Preço: <strong className="text-success">R$ {item.priceValue.toLocaleString('pt-BR')}</strong></span>
        <span>Rating: <strong className="text-foreground">★ {item.ratingScore}</strong></span>
        <span>Vendas: <strong className="text-primary">{item.salesCount} downloads</strong></span>
      </div>
    </ExecutiveSurface>
  );
};
