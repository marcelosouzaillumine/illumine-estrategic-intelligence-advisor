...
export function BoardModeDashboard({ input }: BoardModeDashboardProps) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {/* HEADER SECTION */}
      <div className="bg-slate-900 rounded-3xl p-8 border border-border text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-white/5 border border-white/10 p-5 rounded-2xl">
              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">Cenário Fiduciário</p>
              <p className="text-xl font-black text-white truncate">{input.scenarioName}</p>
            </div>
            <div className="bg-white/5 border border-white/10 p-5 rounded-2xl">
              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">Enterprise Value Projetado</p>
              <p className="text-xl font-black text-emerald-400">{formatCurrency(input.enterpriseValue)}</p>
            </div>
            <div className="bg-white/5 border border-white/10 p-5 rounded-2xl">
              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">Impacto (Delta)</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white rounded-3xl p-6 border border-border shadow-sm">
            <h3 className="text-sm font-black text-muted-foreground uppercase tracking-widest mb-6 flex items-center gap-2">
              <ShieldCheck className="text-muted-foreground" size={16} /> Radar Fiduciário
            </h3>
          ...
          <div className="bg-slate-50 rounded-3xl p-6 border border-border">
            <h3 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-4">Board Pack Generator</h3>
            <div className="p-4 bg-white rounded-xl border border-border text-center">
              <Presentation className="text-muted-foreground mx-auto mb-2" size={32} />
              <p className="text-xs text-muted-foreground font-bold mb-3">Versão Visual (Interactive)</p>
              <button disabled className="w-full py-2 bg-slate-100 text-muted-foreground text-[10px] font-black uppercase tracking-widest rounded-lg cursor-not-allowed border border-border">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-3xl p-6 border border-border shadow-sm">
            <h3 className="text-sm font-black text-muted-foreground uppercase tracking-widest mb-6 flex items-center gap-2">
              <Eye className="text-primary" size={16} /> Radar de Atenção (Monitorar vs Escalar)
            </h3>
              ...
              {attentionItems.map((item, i) => (
                <div key={i} className={cn(
                  ...
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-[9px] font-bold text-muted-foreground bg-white px-2 py-0.5 rounded-full border">{item.urgency}</span>
                  </div>
                  <h4 className="text-xs font-bold text-muted-foreground mb-1">{item.title}</h4>
      ...
      {/* AGENDA PREVIEW */}
      <div className="bg-slate-900 rounded-3xl p-8 border border-border text-white shadow-xl">
        <h3 className="text-sm font-black uppercase tracking-widest mb-8 text-muted-foreground">Pauta Fiduciária Sugerida</h3>
...
function AgendaCol({ block }: { block: any }) {
  return (
    <div>
      <h4 className="text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-4 pb-2 border-b border-border">
        {block.title}
      </h4>
