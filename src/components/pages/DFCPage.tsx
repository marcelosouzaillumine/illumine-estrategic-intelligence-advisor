...
export function DFCPage({ clients, selectedClient, selectedYear }: any) {
  ...
  const cashIndices = [
    ...
    { name: 'Variação Líquida', val: variacao, unit: 'R$', desc: 'Resultado final das movimentações de caixa', color: 'text-muted-foreground' },
  ];
    ...
    >
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap bg-surface-container/60 p-4 rounded-[24px] border border-border backdrop-blur-sm shadow-sm -mt-6 mb-10">
        ...
        <div className="flex items-center gap-3">
          <div className="bg-card border border-border/50 shadow-sm rounded-xl px-4 py-2 flex items-center gap-3 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-3 bg-success-soft0/10 hover:bg-success text-emerald-600 hover:text-white border border-emerald-500/20 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 cursor-pointer"
          >
            <Plus size={14} /> Lançar Dados
          </button>
          <button
            onClick={() => setShowImportModal(true)}
            className="px-4 py-3 bg-secondary/10 hover:bg-secondary text-secondary hover:text-foreground border border-secondary/20 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 cursor-pointer"
          >
            <Upload size={14} /> Importar
          </button>
          <button
            onClick={() => setShowDeleteConfirm(true)}
            className="px-4 py-3 bg-critical-soft hover:bg-destructive text-destructive hover:text-white border border-destructive/20 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 cursor-pointer"
          >
      ...
      <div className="flex justify-center mb-8">
        <div className="bg-foreground/5 p-1.5 rounded-2xl flex gap-1 shadow-inner border border-foreground/10">
          <button
          ...
          <button
            ...
            className={cn(
              ...
              viewMode === 'fiduciario'
                ? "bg-secondary text-foreground shadow-lg scale-105"
                : "text-foreground/60 hover:text-foreground hover:bg-card/60"
            )}
          >
            ...
            {metrics.fiduciary?.isEarlyStage && (
              <span className={cn(
                ...
                viewMode === 'fiduciario'
                  ...
                  : "bg-warning-soft0/10 text-amber-600"
              )}>
                Early
              </span>
            )}
      ...
      {viewMode === 'fiduciario' && (
        <div className="flex justify-center mb-8 animate-in fade-in duration-300">
          <div className="bg-foreground/5 p-1.5 rounded-2xl flex gap-1 shadow-inner border border-foreground/10 text-xs">
            <span className="text-foreground/50 font-black uppercase tracking-widest px-3 py-2 flex items-center select-none text-[10px]">
              Nível EIDF:
            </span>
      ...
      {viewMode === 'fiduciario' && metrics.fiduciary?.lifecycleProfile && (
        <SemanticCard variant="success" className="mb-8 flex flex-col @3xl:flex-row @3xl:items-center justify-between gap-6">
          ...
          <div className="flex gap-8 bg-card/60 p-4 rounded-2xl border border-emerald-100/50">
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Fundação</span>
              <span className="text-lg font-black text-primary">{metrics.fiduciary.lifecycleProfile.foundationYear || 'N/A'}</span>
            </div>
            <div className="w-px bg-emerald-200/50"></div>
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Ano Analisado</span>
              <span className="text-lg font-black text-primary">{filterYear}</span>
            </div>
            <div className="w-px bg-emerald-200/50"></div>
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Idade Empresarial</span>
          ...
          <ul className="grid grid-cols-1 @2xl:grid-cols-2 gap-3 pl-2 mt-2">
            {metrics.fiduciary.governanceWarnings.map((warning: string, idx: number) => (
              <li key={idx} className="flex items-start gap-2.5 text-xs font-semibold text-muted-foreground">
                <span className="w-1.5 h-1.5 rounded-full bg-warning-soft0 mt-1.5 shrink-0" />
                <span>{warning}</span>
              </li>
            ))}
          </ul>
      ...
      {viewMode === 'fiduciario' && (() => {
        ...
        if (densityLevel !== 'TECHNICAL' && (presentationAudit.status === 'DFC_EXECUTIVE_PRESENTATION_VIOLATION' || presentationAudit.status === 'EXECUTIVE_LANGUAGE_LEAK')) {
          ...
          return (
            <div className="max-w-[1440px] mx-auto p-8 text-center bg-critical-soft border border-rose-200 rounded-[32px] my-10">
              ...
              {process.env.NODE_ENV !== 'production' && (
                <div className="mt-4 p-4 bg-foreground text-white rounded-2xl text-left font-mono text-[10px] overflow-auto max-h-48">
        ...
        return (
          <>
            {viewMode === 'fiduciario' && !governanceOutput.validation.isValid && (
              <div className="bg-critical-soft border-2 border-rose-200 rounded-[32px] p-10 mb-10 text-center flex flex-col items-center justify-center animate-in fade-in zoom-in duration-300">
                ...
                <div className="bg-card/60 px-6 py-4 rounded-2xl border border-rose-100 inline-block">
                ...
                <ExecutiveSurface className="bg-gradient-to-br from-foreground via-foreground to-[#07111C] p-8 rounded-[32px] shadow-2xl border border-white/10 space-y-6 text-left relative overflow-hidden">
                  ...
                  <div className="grid grid-cols-1 @2xl:grid-cols-2 @4xl:grid-cols-3 gap-6 relative z-10">
                    <div className="bg-card/5 p-5 rounded-2xl border border-white/10 shadow-sm hover:bg-card/10 transition-colors">
                      <span className="text-[9px] font-black uppercase text-muted-foreground tracking-wider">Geração de Caixa</span>
                      <p className={cn("text-lg font-black mt-1", snapshot.geraCaixa ? "text-emerald-400" : "text-destructive")}>
                    ...
                    <div className="bg-card/5 p-5 rounded-2xl border border-white/10 shadow-sm hover:bg-card/10 transition-colors">
                      <span className="text-[9px] font-black uppercase text-muted-foreground tracking-wider">Dependência de Aportes</span>
                      <p className="text-lg font-black text-white mt-1">
                ...
                >
                  <div className="space-y-4">
                    {adaptedPriorities.slice(0, 3).map((d: any, idx: number) => {
                      ...
                      return (
                        <div key={idx} className="flex items-start gap-4 p-4 bg-surface-container/30 border border-border rounded-2xl flex-col @3xl:flex-row @3xl:items-start">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              <span className="text-[8px] font-black uppercase tracking-widest text-muted-foreground">{d.theme}</span>
                              <span className="text-[7px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded bg-primary text-primary border border-primary">
                                {d.impact}
                              </span>
                            </div>
                            ...
                            {densityLevel === 'TECHNICAL' && (
                              <details className="mt-2 text-[8px] font-mono text-muted-foreground cursor-pointer select-none">
                                <summary className="hover:text-muted-foreground font-bold uppercase tracking-wider">Auditoria Técnica (Debug)</summary>
                                <div className="mt-1 p-2 bg-surface-container border border-border rounded-lg space-y-1 text-[8.5px]">
            ...
            {isSectionVisible('DFC_EXECUTIVE_DIAGNOSIS') && (
              ...
              >
                ...
                <div className="grid grid-cols-1 @3xl:grid-cols-2 gap-6 mt-4">
                  <div className="border border-border rounded-2xl p-5 space-y-1 bg-card shadow-sm hover:shadow-md transition-all duration-300">
                    <span className="text-[9px] font-black uppercase text-foreground/50 tracking-wider">A operação gera caixa operacional?</span>
                  ...
                  <div className="border border-border rounded-2xl p-5 space-y-1 bg-card shadow-sm hover:shadow-md transition-all duration-300">
                    <span className="text-[9px] font-black uppercase text-foreground/50 tracking-wider">Qual a restrição principal de caixa?</span>
                    <p className="text-xs font-bold text-foreground leading-relaxed">
                  ...
                  <div className="border border-border rounded-2xl p-5 space-y-1 bg-card shadow-sm hover:shadow-md transition-all duration-300">
                    <span className="text-[9px] font-black uppercase text-foreground/50 tracking-wider">Qual o horizonte de sobrevivência (runway)?</span>
                    <p className="text-xs font-bold text-foreground leading-relaxed">
                  ...
                  <div className="border border-border rounded-2xl p-5 space-y-1 bg-card shadow-sm hover:shadow-md transition-all duration-300">
                    <span className="text-[9px] font-black uppercase text-foreground/50 tracking-wider">Qual a dependência dos sócios?</span>
                    <p className="text-xs font-bold text-foreground leading-relaxed">
                  ...
                  <div className="border border-border rounded-2xl p-5 space-y-1 bg-card shadow-sm hover:shadow-md transition-all duration-300 col-span-1 @3xl:col-span-2">
                    <span className="text-[9px] font-black uppercase text-foreground/50 tracking-wider">Qual a perspectiva (outlook) de continuidade?</span>
                    <p className="text-xs font-bold text-foreground leading-relaxed">
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 @2xl:grid-cols-2 gap-6 mt-4">
                     ...
                     <div className="space-y-2">
                        <span className="text-[10px] font-black uppercase tracking-widest text-destructive">Vetor de Consumo</span>
                        {governanceOutput.explainability.consumers.map((c: string, idx: number) => (
                          <div key={idx} className="p-3 bg-critical-soft text-rose-800 text-xs font-semibold rounded-lg border border-rose-100">
                            {c}
                          </div>
                        ))}
                     </div>
            ...
            {isSectionVisible('DFC_REVENUE_CASH_CONVERSION') && (
              ...
              >
                <div className="flex flex-col @3xl:flex-row items-stretch gap-6 bg-foreground/5 p-6 rounded-[24px] border border-foreground/10 shadow-sm">
                  ...
                  <div className="bg-card border border-foreground/10 px-8 py-6 rounded-2xl text-center min-w-[220px] shrink-0 shadow-sm flex flex-col justify-center">
                    <span className="text-[10px] font-black text-foreground/50 uppercase tracking-widest block mb-2">Conversão de Faturamento</span>
                    <span className="text-3xl font-black text-foreground tracking-tighter block">
                    ...
                    {governanceOutput.narratives.cashConversionAnalysis ? (
                      <span className="text-[9px] font-bold text-foreground/50 uppercase block mt-1">A cada R$ 100 faturados</span>
                    ) : (
                      <span className="text-[9px] font-bold text-foreground/50 uppercase block mt-1">Sem base de faturamento disponível</span>
                    )}
                  </div>
                </div>
              </SemanticCard>
                ...
                <div className="bg-gradient-to-br from-foreground to-[#07111C] text-white p-8 rounded-[24px] border border-white/10 shadow-xl relative overflow-hidden">
                  ...
                  <div className="relative z-10 flex flex-col @3xl:flex-row @3xl:items-stretch justify-between gap-8">
                    <div className="space-y-2 flex-1 text-left flex flex-col justify-center">
                      <span className="text-[10px] font-black text-secondary uppercase tracking-widest block">Diagnóstico de Sobrevivência</span>
                      <h4 className="text-lg font-bold text-primary leading-relaxed">
                        {metrics.fiduciary?.cashBoardDecisionFramework?.runwayAssessment}
                      </h4>
                    </div>
                    <div className="bg-card/5 border border-white/10 px-8 py-6 rounded-2xl text-center min-w-[220px] shrink-0 shadow-lg flex flex-col justify-center">
                      <span className="text-[10px] font-black text-white/50 uppercase tracking-widest block mb-2">Cobertura Projetada</span>
                      <span className="text-4xl font-black text-secondary tracking-tighter block">
            ...
            {isSectionVisible('DFC_SCENARIO_SIMULATION') && metrics.fiduciary?.scenarioIntelligence && (() => {
              ...
              return (
                ...
                >
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          selectedScenarioIndex === idx
                            ...
                            : "border-transparent text-foreground/50 hover:text-foreground hover:bg-foreground/5 rounded-t-xl"
                        )}
                      >
                        {g.scenarioName}
                      </button>
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-semibold text-foreground/60 uppercase tracking-wide">Intensidade:</span>
                  ...
                  <div className="grid grid-cols-1 @3xl:grid-cols-3 gap-6 bg-foreground/5 p-6 rounded-2xl border border-foreground/10">
                    <div className="space-y-1">
                      <span className="text-[10px] font-bold text-foreground/50 uppercase tracking-widest">Caixa Disponível</span>
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-foreground/70">Atual: {formatCurrency(currentCash)}</span>
                        <span className="font-bold text-foreground">Simulado: {formatCurrency(sim.cashSimulated)}</span>
                      </div>
                    </div>
                    <div className="space-y-1">
                      <span className="text-[10px] font-bold text-foreground/50 uppercase tracking-widest">Runway Estimado</span>
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-foreground/70">Atual: {currentRunway >= 99 ? '99+' : currentRunway.toFixed(1)} meses</span>
                        <span className="font-bold text-secondary">Simulado: {sim.runwayDisplay}</span>
                      </div>
                    </div>
                    <div className="space-y-1">
                      <span className="text-[10px] font-bold text-foreground/50 uppercase tracking-widest">Fluxo Operacional (FCO)</span>
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-foreground/70">Atual: {formatCurrency(currentFco)}</span>
                        <span className="font-bold text-foreground">Simulado: {formatCurrency(sim.fcoSimulated)}</span>
                      </div>
                    </div>
                  </div>
                        ...
                        <div>
                          ...
                          <div>
                            <span className={cn(
                              ...
                              consequence.reversibility.startsWith('Alta') ? 'bg-success-soft0/20 text-emerald-700 border border-emerald-500/30' :
                              consequence.reversibility.startsWith('Recuperação possível') ? 'bg-warning-soft0/20 text-amber-700 border border-amber-500/30' :
                              'bg-critical-soft0/20 text-rose-700 border border-rose-500/30'
                            )}>
                ...
                actions={
                  <span className={cn(
                    ...
                    metrics.fiduciary?.reconciliationMismatch 
                      ? "bg-critical-soft text-destructive border border-destructive/20" 
                      : "bg-success-soft0/10 text-emerald-600 border border-emerald-500/20"
                  )}>
                ...
                <div className="grid grid-cols-1 @3xl:grid-cols-2 gap-6 bg-foreground/5 p-6 rounded-2xl border border-foreground/10">
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between text-xs font-medium">
                      <span className="text-foreground/70">Caixa Inicial DFC:</span>
                      <span className="text-foreground font-semibold">{formatCurrency(metrics.fiduciary?.caixaInicialDFC || 0)}</span>
                    </div>
                    <div className="flex justify-between text-xs font-medium">
                      <span className="text-foreground/70">Variação DFC:</span>
                      <span className="text-foreground font-semibold">{formatCurrency(metrics.fiduciary?.variacaoDFC || 0)}</span>
                    </div>
                    <div className="flex justify-between text-xs font-bold border-t border-foreground/10 pt-2">
                      <span className="text-foreground">Caixa Final Estimado:</span>
                      <span className="text-foreground font-bold">{formatCurrency(metrics.fiduciary?.caixaFinalEstimadoDFC || 0)}</span>
                    </div>
                  </div>
                  <div className="space-y-3">
                    <span className="text-[10px] font-black uppercase tracking-widest text-foreground/50 block mb-1">Valores BP (Real)</span>
                    <div className="flex justify-between text-xs font-medium">
                      <span className="text-foreground/70">Caixa Inicial Real (BP):</span>
                      <span className="text-foreground font-semibold">{formatCurrency(metrics.fiduciary?.caixaInicialBP || 0)}</span>
                    </div>
                    <div className="flex justify-between text-xs font-medium">
                      <span className="text-foreground/70">Variação Real (BP):</span>
                      <span className="text-foreground font-semibold">{formatCurrency(metrics.fiduciary?.variacaoLiquidaConciliada || 0)}</span>
                    </div>
                    <div className="flex justify-between text-xs font-bold border-t border-foreground/10 pt-2">
                      <span className="text-foreground">Caixa Final Real (BP):</span>
                      <span className="text-foreground font-bold">{formatCurrency(metrics.fiduciary?.caixaFinalBP || 0)}</span>
                    </div>
                  </div>
                </div>
              </SemanticCard>
              ...
              >
                ...
                <div className="grid grid-cols-1 @xl:grid-cols-2 @5xl:grid-cols-3 gap-8 items-center">
                  <div className="bg-foreground/5 border border-foreground/10 rounded-[32px] p-8 flex flex-col items-center justify-center text-center relative overflow-hidden h-full min-h-[300px]">
                    <span className="text-[10px] font-black uppercase tracking-widest text-foreground/50 mb-6">Nível de Sustentabilidade dos Resultados</span>
                    <div className="relative w-36 h-36 flex items-center justify-center">
                      ...
                      <div className="text-center z-10">
                        <span className="text-4xl font-black text-foreground tracking-tighter">{earningsQuality.score}</span>
                        <span className="text-sm font-bold text-foreground/40">/100</span>
                      </div>
                    </div>
                  ...
                  <div className="bg-foreground/5 border border-foreground/10 rounded-[32px] p-4 flex items-center justify-center h-full min-h-[300px]">
                    <div className="w-full h-[280px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <RadarChart cx="50%" cy="50%" outerRadius="65%" data={earningsRadarData}>
                          ...
                          <Radar name="Score" dataKey="A" stroke="var(--secondary)" fill="var(--secondary)" fillOpacity={0.15} strokeWidth={2} />
                        </RadarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-4 justify-center h-full">
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-foreground/50">Alertas de Qualidade do Lucro</h4>
                    {earningsQuality.alerts.length === 0 ? (
                      <div className="bg-success-soft0/5 border border-emerald-500/10 p-5 rounded-2xl flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-success-soft0/10 flex items-center justify-center text-emerald-700 shrink-0">
                          <Info size={16} />
                        </div>
                      ...
                      <div className="space-y-3 max-h-[250px] overflow-y-auto pr-1">
                        {earningsQuality.alerts.filter((alert: string) => !['LOSS_WITH_CASH_CONSUMPTION', 'PROFIT_WITHOUT_CASH', 'NET_INCOME_SOURCE_MISSING', 'PREJUIZO_OPERACIONAL'].includes(alert)).map((alert: string, idx: number) => (
                          <div key={idx} className="bg-[#BAB86C]/10 border border-[#BAB86C]/25 p-4 rounded-2xl flex items-start gap-3">
                            ...
                            <div className="text-left">
                              <p className="text-xs font-bold text-foreground">Ponto de Atenção</p>
                ...
                {isSectionVisible('DFC_EQE_LINEAGE') && (
                  <div className="border-t border-border pt-8 space-y-4">
                    ...
                    {earningsQuality?.netIncomeTrace && (
                      <div className="bg-foreground/5 border border-foreground/10 rounded-3xl p-5 mt-4 flex flex-col @3xl:flex-row @3xl:items-center justify-between gap-4 shadow-xs text-foreground">
                        <div>
                    ...
                    <div className="grid grid-cols-1 @xl:grid-cols-2 @4xl:grid-cols-3 gap-4">
                      {Object.entries(earningsQuality?.dimensions || {}).map(([key, dim]: [string, any]) => {
                        ...
                        else if (key === 'sustainability') { title = 'Sustentabilidade da Margem'; colorClass = 'text-insight'; }
                          ...
                          <div 
                            ...
                            className={cn(
                              ...
                              isExpanded 
                                ...
                                : "border-foreground/10 bg-foreground/5 hover:bg-foreground/10 hover:border-foreground/20 text-foreground"
                            )}
                          >
                            <div className="flex items-center justify-between">
                              <div>
                                <p className={cn("text-[9px] font-black uppercase tracking-wider", isExpanded ? "text-white/60" : colorClass)}>{title}</p>
                                <p className={cn("text-[10px] font-semibold mt-1", isExpanded ? "text-white/80" : "text-foreground/70")}>Score: {dim.score}</p>
                              </div>
                            </div>
                            {isExpanded && (
                              <div className="mt-4 pt-4 border-t border-white/10 space-y-3.5 text-xs animate-in fade-in duration-300">
                                <div>
                                  ...
                                  <code className="block bg-black/30 p-2 rounded-lg mt-1 font-mono text-[10px] text-primary break-all">{dim.formula}</code>
                                </div>
                                <div>
      ...
      {viewMode === 'oficial' && (
        <div className="bg-card border border-foreground/10 rounded-[32px] shadow-sm overflow-hidden mb-10">
          <div className="px-5 md:px-8 py-4 md:py-6 border-b border-foreground/10 flex items-center justify-between bg-foreground/5">
            <h4 className="text-sm font-black text-foreground uppercase tracking-widest">
              Detalhamento da DFC
            </h4>
            <span className="text-[9px] font-black uppercase px-3 py-1 rounded-full bg-secondary/10 text-secondary border border-secondary/20">
                   ...
                   return (
                     <ExecutiveTableRow 
                       ...
                       className={cn(
                         ...
                         isTotalRow ? 'bg-foreground/5 font-bold text-foreground' : '',
                         isReclassified ? 'bg-warning-soft0/5 hover:bg-warning-soft0/10' : ''
                       )}
                     >
                       <td className="py-3 px-5 md:px-8">
                         <span className={cn(
                           'block overflow-visible break-words flex items-center gap-1.5 flex-wrap', 
                           (isTotalRow || row.level === 1) ? 'text-foreground font-bold' : 'text-foreground/80 font-medium',
                       ...
                       <td className={cn(
                         ...
                         (row.val || row.valor || row.value || 0) < 0 
                           ? "text-destructive" 
                           : (isTotalRow ? "text-foreground font-bold" : "text-foreground/80")
                       )}>
                         {formatCurrency(row.val || row.valor || row.value || 0)}
                       </td>
                     </ExecutiveTableRow>
