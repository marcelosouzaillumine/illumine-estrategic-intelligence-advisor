...
export function ComplianceIntegrityCenter() {
  ...
  return (
    <ExecutivePageTemplate header={{ title: t("compliance.header.title"), description: t("compliance.header.description"), icon: ShieldAlert }}>
      ...
      <ExecutiveSummarySection 
        ...
        executiveQuestion="Tratar imediatamente as denúncias abertas e mitigar os gaps de compliance."
      >
      ...
      {activeTab === 'integridade' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in duration-300 mt-6">
          ...
          <ExecutiveSurface variant="default" padding="lg">
            ...
            <div className="space-y-4">
              <RiskItem title="Atraso na renovação da Política Anticorrupção" severity={t('executive:status.critical')} />
              <RiskItem title="Aumento de denúncias de assédio no setor logístico" severity={t('executive:status.critical')} />
            </div>
          </ExecutiveSurface>
        </div>
      )}
      ...
      {activeTab === 'denuncias' && (
        <ExecutiveSurface variant="default" padding="lg" className="animate-in fade-in duration-300 mt-6 space-y-4">
          ...
          <ReportItem protocol="REL-2026-089" category="Fraude Financeira" severity={t('executive:status.critical')} status="Em Investigação" date="Há 2 dias" />
          <ReportItem protocol="REL-2026-074" category="Conflito de Interesses" severity={t('executive:status.attention')} status="Triagem" date="Há 5 dias" />
        </ExecutiveSurface>
      )}
      ...
      {activeTab === 'politicas' && (
        <ExecutiveSurface variant="default" padding="lg" className="animate-in fade-in duration-300 mt-6 space-y-4">
          ...
          <PolicyItem title="Código de Conduta Ética" compliance="94%" status={t('executive:status.healthy')} />
          <PolicyItem title="Política de Segurança da Informação" compliance="78%" status={t('executive:status.attention')} />
          <PolicyItem title="Diretriz Anticorrupção & Bribery" compliance="99%" status={t('executive:status.healthy')} />
        </ExecutiveSurface>
      )}
...
function RiskItem({ title, severity }: { title: string, severity: string }) {
  const { t } = useTranslation('executive');
  const isCritical = severity === t('executive:status.critical');
  return (
...
function PolicyItem({ title, compliance, status }: { title: string, compliance: string, status: string }) {
  const { t } = useTranslation('executive');
  const isAttention = status === t('executive:status.attention');
  return (
    <div className="p-4 bg-muted/20 border border-border/40 rounded-xl flex justify-between items-center">
      <div className="flex items-center gap-3">
