...
export function OrcamentoPage({ 
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      setBudgets(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as BudgetEntry)));
      setLoading(false);
    });
    return () => unsubscribe();
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-slate-900 p-8 rounded-[32px] text-white shadow-2xl relative overflow-hidden mb-12">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-display font-black tracking-tight">Orçamento & Budget</h1>
          </div>
          <p className="text-slate-400 text-sm font-medium whitespace-nowrap">Planejamento financeiro vinculado a unidades, filiais e centros de custo.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-end gap-3 relative z-10">
          <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10 mr-4">
            <button 
              ...
              className={cn(
                ...
                viewType === 'mensal' ? "bg-secondary text-white shadow-lg" : "text-slate-400 hover:text-white"
              )}
            >
              Mensal
            </button>
            <button 
              ...
              className={cn(
                "px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all",
                viewType === 'anual' ? "bg-secondary text-white shadow-lg" : "text-slate-400 hover:text-white"
              )}
            >
              Anual
            </button>
          ...
          <button 
            ...
            className="px-6 py-3 bg-white/5 text-emerald-400 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-500/10 transition-all flex items-center gap-2"
          >
            <LayoutGrid size={16} /> DUPLICAR MÊS
          </button>

          <label className="cursor-pointer px-6 py-3 bg-white/5 text-slate-300 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all flex items-center gap-2">
