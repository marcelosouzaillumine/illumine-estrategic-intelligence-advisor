...
export function CausalityExplorerPanel({ causality }: CausalityExplorerPanelProps) {
  ...
  if (!causality) {
    return (
      <div className="p-6 rounded-[32px] border border-slate-200 dark:border-zinc-900 bg-slate-50/50 dark:bg-zinc-950/40 backdrop-blur-md text-center py-10">
        <Network className="w-8 h-8 text-slate-400 mx-auto mb-3 animate-pulse" />
  ...
  const getSeverityBadgeClass = (severity: CausalChain['severity']) => {
    switch (severity) {
      ...
      case 'INFO':
      default:
        return 'bg-slate-100 dark:bg-zinc-900 text-slate-600 dark:text-zinc-400 border-slate-200 dark:border-zinc-800';
    }
  };
  ...
  return (
    <div className="w-full p-6 md:p-8 rounded-[32px] border border-slate-200 dark:border-zinc-900 bg-slate-50/50 dark:bg-zinc-950/40 backdrop-blur-md space-y-8 shadow-xs">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-white/5 pb-5">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-indigo-500/10 border border-indigo-500/20 rounded-xl text-indigo-550 dark:text-indigo-400">
            <Network className="w-5 h-5" />
          </div>
          <div>
            <h3 className="text-sm font-black text-slate-800 dark:text-zinc-200 uppercase tracking-widest">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={cn(
            ...
            confidenceLevel === 'MEDIUM' ? 'bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border-indigo-500/20' :
      ...
      <div className={cn(
        ...
        isRestricted 
          ...
          : "bg-slate-100/50 dark:bg-zinc-900/30 border-slate-200 dark:border-zinc-800"
      )}>
        <div className="space-y-4">
          <div className="flex items-start gap-4">
            {isRestricted ? (
              ...
              <ShieldCheck className="w-5 h-5 text-indigo-550 dark:text-indigo-400 shrink-0 mt-0.5" />
            )}
            <div className="space-y-1 flex-1">
              <span className="text-[9px] font-black uppercase tracking-widest text-slate-500 dark:text-zinc-500">
                {t('runtime.ice.panel.narrative_label') || 'Diagnóstico Narrativo de Causalidade'}
              </span>
              <p className="text-xs font-semibold text-slate-800 dark:text-zinc-300 leading-relaxed">
                {executiveNarrative}
              </p>
            </div>
          </div>
          ...
          {!isRestricted && primaryCause && (
            <div className="pt-4 border-t border-slate-200 dark:border-white/5 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-550 dark:text-zinc-500">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {secondaryCauses.map((sc, i) => (
                      <span key={i} className="text-[9px] font-bold text-slate-700 dark:text-zinc-400 bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 px-2 py-0.5 rounded">
                        {translateFactor(sc)}
                      </span>
                    ))}
                  </div>
      ...
      <div className="space-y-4">
        <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-550 dark:text-zinc-500">
          {t('runtime.ice.panel.causal_chains_label') || 'Cadeias de Causa e Efeito Detectadas'}
        </h4>

        {causalChains.length === 0 ? (
          <div className="p-4 rounded-xl border border-slate-200 dark:border-zinc-900 bg-slate-50/50 dark:bg-zinc-950/20 text-center text-[10px] font-medium text-slate-500 uppercase tracking-wider">
            {t('runtime.ice.panel.no_causes') || 'Nenhuma cadeia de causa e efeito fiduciária foi disparada.'}
          </div>
        ) : (
          <div className="space-y-4">
            {causalChains.map((chain, index) => (
              <div key={index} className="p-5 bg-slate-100/30 dark:bg-zinc-900/10 border border-slate-200 dark:border-zinc-900/60 rounded-2xl space-y-4 hover:border-slate-350 dark:hover:border-zinc-800 transition-all shadow-xs">
                
                {/* Chain Tags */}
                <div className="flex justify-between items-center gap-4 flex-wrap">
                  <div className="flex items-center gap-2">
                    <span className="text-[8px] font-black uppercase tracking-widest px-2.5 py-0.5 bg-indigo-500/10 text-indigo-755 dark:text-indigo-400 border border-indigo-500/20 rounded">
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-3 py-2">
                  ...
                  {/* Cause */}
                  <div className="flex-1 p-3 bg-slate-50/90 dark:bg-zinc-950/50 border border-slate-200 dark:border-zinc-900 rounded-xl flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <span className="text-[8px] font-black text-slate-400 dark:text-zinc-550 uppercase tracking-widest block leading-none mb-1">
                        Causa Raiz
                      </span>
                      <span className="text-[11px] font-bold text-slate-800 dark:text-zinc-200 block truncate">
                        {translateFactor(chain.cause)}
                      </span>
                    </div>
                  </div>

                  <div className="flex items-center justify-center text-slate-400 dark:text-zinc-600 shrink-0">
                  ...
                  {/* Effect */}
                  <div className="flex-1 p-3 bg-slate-50/90 dark:bg-zinc-950/50 border border-slate-200 dark:border-zinc-900 rounded-xl flex items-center gap-3">
                    <div className="w-6 h-6 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-550 dark:text-indigo-400 flex items-center justify-center shrink-0">
                ...
                {/* Chain narrative */}
                <div className="p-3 bg-slate-50/50 dark:bg-zinc-950/30 border border-slate-200/60 dark:border-zinc-900/60 rounded-xl text-[11px] font-semibold text-slate-650 dark:text-zinc-400 leading-relaxed">
                  {chain.narrative}
                </div>
        ...
        <div className="space-y-3 pt-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3.5">
            {supportingEvidence.map((ev, i) => {
              ...
              return (
                <div key={i} className="p-3.5 bg-slate-100/50 dark:bg-zinc-900/30 border border-slate-200 dark:border-zinc-900 rounded-xl space-y-1 text-center shadow-xs">
                  <span className="text-[8px] font-black text-slate-400 dark:text-zinc-550 uppercase tracking-wider block">
                    {ev.metricName}
                  </span>
                  <span className="text-[11px] font-black text-slate-850 dark:text-zinc-300 block">
                    {formattedValue}
                  </span>
                  <span className="text-[7px] font-mono text-slate-400 dark:text-zinc-600 block">
      ...
      {/* Causal Lineage Hash */}
      <div className="p-3 bg-slate-100/90 dark:bg-zinc-950/80 border border-slate-200 dark:border-zinc-900 rounded-xl flex items-center justify-between text-[8px] font-mono text-slate-500 dark:text-zinc-500 shadow-inner">
