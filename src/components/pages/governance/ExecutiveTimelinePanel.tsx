...
export function ExecutiveTimelinePanel({ timeline }: ExecutiveTimelinePanelProps) {
  ...
  if (!timeline) {
    return (
      <div className="p-6 rounded-[32px] border border-border dark:border-zinc-900 bg-slate-50/50 dark:bg-zinc-950/40 backdrop-blur-md text-center py-10 text-muted-foreground dark:text-zinc-500 shadow-xs">
        <History className="w-8 h-8 mx-auto text-muted-foreground dark:text-zinc-700 mb-2" />
        <p className="text-xs font-bold uppercase tracking-wider">Histórico Longitudinal Indisponível</p>
        <p className="text-[10px] text-muted-foreground dark:text-zinc-600 mt-1">Nenhum dado temporal de linha do tempo fiduciária foi gerado.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 rounded-[32px] border border-border dark:border-zinc-900 bg-slate-50/50 dark:bg-zinc-950/40 backdrop-blur-md space-y-6 shadow-xs">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-4 rounded-2xl bg-slate-100/65 dark:bg-zinc-900/20 border border-border dark:border-zinc-900 space-y-1 shadow-xs">
          <span className="text-[8px] font-black uppercase tracking-widest text-muted-foreground dark:text-zinc-500 block">
            {t('runtime.ete.panel.acceleration_state', 'Estado de Aceleração')}
          </span>
          <span className="text-sm font-bold text-muted-foreground dark:text-zinc-200 uppercase tracking-wider">
            {t(`runtime.ete.acceleration.${accelerationState}`, accelerationState.replace(/_/g, ' '))}
          </span>
        </div>

        <div className="p-4 rounded-2xl bg-slate-100/65 dark:bg-zinc-900/20 border border-border dark:border-zinc-900 flex flex-col justify-center space-y-1 min-w-0 shadow-xs">
          <span className="text-[8px] font-black uppercase tracking-widest text-muted-foreground dark:text-zinc-500 block">
            {t('runtime.ete.panel.lineage_hash', 'Hash de Linhagem Temporal')}
          </span>
          <div className="flex items-center gap-1.5 text-muted-foreground dark:text-zinc-400 font-mono text-[9px] min-w-0">
            <Hash className="w-3.5 h-3.5 text-muted-foreground dark:text-zinc-500 shrink-0" />
      ...
      {/* Narrative Section */}
      <div className="p-4 rounded-xl bg-slate-100/80 dark:bg-zinc-950/60 border border-border dark:border-zinc-900 space-y-1 shadow-xs">
        ...
        <p className="text-xs font-semibold text-muted-foreground dark:text-zinc-300 leading-relaxed">{executiveNarrative}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-2">
        ...
        <div className="space-y-3">
          ...
          {timelineEvents.length === 0 ? (
            <div className="p-4 rounded-2xl bg-slate-100/50 dark:bg-zinc-950/40 border border-border dark:border-zinc-900 text-center text-[10px] text-muted-foreground dark:text-zinc-500 font-semibold uppercase tracking-wider shadow-xs">
            ...
            <div className="space-y-2.5 max-h-[300px] overflow-y-auto pr-2 no-scrollbar">
              {timelineEvents.map((evt, idx) => {
                ...
                return (
                  <div key={idx} className="p-3 bg-slate-100/80 dark:bg-zinc-950/60 border border-border dark:border-zinc-900 rounded-xl space-y-1.5 transition-colors hover:border-border dark:hover:border-zinc-800 shadow-xs">
                    <div className="flex items-center justify-between text-[9px] font-mono">
                      <span className="font-bold text-muted-foreground dark:text-zinc-400">{evt.cycleReference}</span>
          ...
          {inflectionPoints.length === 0 ? (
            ...
            <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 no-scrollbar">
              {inflectionPoints.map((ip, idx) => {
                ...
                return (
                  <div key={idx} className="p-3 bg-slate-100/80 dark:bg-zinc-950/60 border border-border dark:border-zinc-900 rounded-xl flex items-center justify-between gap-4 transition-colors hover:border-border dark:hover:border-zinc-800 shadow-xs">
                    <div className="space-y-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className="text-[9px] font-mono font-bold text-muted-foreground dark:text-zinc-500">{ip.cycleReference}</span>
                        <span className="text-[10px] font-bold text-muted-foreground dark:text-zinc-300 truncate uppercase tracking-wide">
                          {ip.metricName.replace(/([A-Z])/g, ' $1')}
                        </span>
                      </div>
                      <div className="flex items-center gap-1.5 text-[10px] font-mono text-muted-foreground dark:text-zinc-400">
                        <span className="text-muted-foreground dark:text-zinc-500">{ip.previousValue}</span>
                        <ChevronRight className="w-3 h-3 text-muted-foreground dark:text-zinc-600" />
                        <span className="font-bold text-muted-foreground dark:text-zinc-300">{ip.newValue}</span>
                      </div>
                    </div>
