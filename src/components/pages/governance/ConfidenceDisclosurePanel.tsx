    ...
    <div className={`p-6 rounded-[32px] border ${theme.border} ${theme.bg} ${theme.glow} space-y-4 shadow-xs`}>
      ...
      {confidenceDecomposition && Object.keys(confidenceDecomposition).length > 0 && (
        <div className="space-y-3">
          <span className="block text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-zinc-500 mb-1">
            Fatores de Decomposição
          </span>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {Object.entries(confidenceDecomposition).map(([factor, desc], idx) => (
              <div key={idx} className="p-3.5 bg-slate-50/50 dark:bg-zinc-950/40 border border-slate-200 dark:border-zinc-900 rounded-xl flex flex-col gap-1 shadow-xs">
                <span className="text-[9px] font-black uppercase tracking-wider text-slate-500 dark:text-zinc-400">
                  {factor.replace(/_/g, ' ')}
                </span>
                <p className="text-[11px] font-medium text-slate-600 dark:text-zinc-400 leading-snug">
                  {desc}
                </p>
              </div>
            ))}
        ...
        <div className="space-y-2.5 pt-2 border-t border-slate-200 dark:border-white/5">
          ...
          <div className="space-y-2">
            {Object.entries(rationaleMap).map(([key, value], idx) => (
              <div key={idx} className="text-xs font-semibold text-slate-700 dark:text-zinc-300 leading-relaxed pl-3 border-l-2 border-slate-200 dark:border-zinc-800">
                <span className="text-slate-500 dark:text-zinc-500 font-mono text-[10px] uppercase block mb-0.5">{key}:</span>
                {value}
              </div>
            ))}
          </div>
