...
export function InstitutionalMemoryCenter({ selectedClient, selectedYear }: InstitutionalMemoryCenterProps) {
  const { state, computed, actions } = useInstitutionalMemoryViewModel({ selectedClient, selectedYear });
  const { expandedSection, loading } = state;
  const {
    imsScore,
    imsLevel,
