      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-black/20 border border-gray-800/80 rounded-2xl p-6 flex flex-col items-center justify-between text-center relative overflow-hidden">
          <div className="absolute top-2 left-2 flex items-center gap-1">
            ...
            <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Score Dial</span>
          </div>
          ...
          <div className="relative flex items-center justify-center my-6">
            ...
            <div className="absolute flex flex-col items-center justify-center">
              ...
              <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-0.5">Pontos PAI™</span>
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6 flex flex-col justify-between">
          <div className="bg-black/20 border border-gray-800/80 rounded-2xl p-6 space-y-4">
            ...
            <div className="text-[11px] text-gray-500 font-mono bg-black/40 p-2.5 rounded border border-gray-800 break-all">
              <span className="text-indigo-400 font-bold block mb-1">LIN-GKL HASH:</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className={`p-4 rounded-xl border text-xs ${scenario === 'CONSTITUTIONAL_BREACH' ? 'bg-red-500/10 border-red-500/30' : 'bg-gray-950/20 border-gray-850'}`}>
              <div className="flex items-center gap-2 font-bold mb-1">
                <Lock className={`w-3.5 h-3.5 ${scenario === 'CONSTITUTIONAL_BREACH' ? 'text-red-400' : 'text-gray-500'}`} />
                <span className={scenario === 'CONSTITUTIONAL_BREACH' ? 'text-red-400' : 'text-gray-400'}>Constitutional Breach Cap</span>
              </div>
              <p className="text-[10px] text-gray-500">PAI limitado a **34** caso haja violações de cláusulas pétreas ou alçadas fiduciárias.</p>
            </div>

            <div className={`p-4 rounded-xl border text-xs ${scenario === 'LIQUIDITY_SHOCK' ? 'bg-orange-500/10 border-orange-500/30' : 'bg-gray-950/20 border-gray-850'}`}>
              <div className="flex items-center gap-2 font-bold mb-1">
                <AlertTriangle className={`w-3.5 h-3.5 ${scenario === 'LIQUIDITY_SHOCK' ? 'text-orange-400' : 'text-gray-500'}`} />
                <span className={scenario === 'LIQUIDITY_SHOCK' ? 'text-orange-400' : 'text-gray-400'}>Liquidity Shock Cap</span>
              </div>
              <p className="text-[10px] text-gray-500">PAI limitado a **48** devido ao estresse extremo de sobrevivência e runway fiduciário.</p>
            </div>

            <div className={`p-4 rounded-xl border text-xs ${scenario === 'MISSION_STRESS' ? 'bg-amber-500/10 border-amber-500/30' : 'bg-gray-950/20 border-gray-850'}`}>
              <div className="flex items-center gap-2 font-bold mb-1">
                <Compass className={`w-3.5 h-3.5 ${scenario === 'MISSION_STRESS' ? 'text-amber-400' : 'text-gray-500'}`} />
                <span className={scenario === 'MISSION_STRESS' ? 'text-amber-400' : 'text-gray-400'}>Mission Stress Cap</span>
              </div>
              <p className="text-[10px] text-gray-500">PAI limitado a **55** devido ao desalinhamento fiduciário do legado principal.</p>
            </div>
          </div>
        </div>
              ...
              return (
                <button
                  ...
                  className={`p-3 rounded-xl border text-left transition-all relative ${
                    isSelected 
                      ? 'bg-indigo-500/15 border-indigo-500/50 text-indigo-300 ring-2 ring-indigo-500/20' 
                      : activeCount > 0
                        ? 'bg-gray-950/40 border-gray-800 hover:border-gray-700 text-gray-300'
                        : 'bg-gray-950/10 border-gray-850 opacity-40 hover:opacity-60 text-gray-500'
                  }`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-black tracking-wider uppercase">{cat.label}</span>
                    {activeCount > 0 && (
                      <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${isSelected ? 'bg-indigo-500/30 text-indigo-300' : 'bg-gray-800 text-indigo-400'}`}>
                        {activeCount}
                      </span>
                    )}
                  </div>
                  <p className="text-[9px] text-gray-400 leading-snug line-clamp-1">{cat.desc}</p>
                </button>
              );
            })}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredMatches.map((pm, idx) => (
              <div 
                key={idx} 
                className="p-5 bg-black/25 border border-gray-800 hover:border-gray-750 transition-colors rounded-2xl flex items-start gap-4 shadow-sm"
              >
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-black uppercase tracking-widest text-indigo-400 bg-indigo-500/10 px-2 py-0.5 rounded border border-indigo-500/25">
                      {pm.category}
                    </span>
                  ...
                  <div className="flex items-center gap-1.5 text-[10px] text-gray-550 pt-1">
            ...
            {filteredMatches.length === 0 && (
              <div className="col-span-2 text-center p-8 bg-black/15 border border-gray-800 rounded-2xl border-dashed">
