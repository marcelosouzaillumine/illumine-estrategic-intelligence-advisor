  ...
  return (
    <div className="max-w-[1440px] mx-auto px-6 lg:px-10 space-y-12 pb-32 animate-executive-fade">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        {/* SDS Health */}
        <div className="bg-white border border-border rounded-3xl p-6 relative overflow-hidden shadow-sm">
        ...
        <div className="bg-white border border-border rounded-3xl p-6 shadow-sm">
          ...
          <div className="mt-4 h-1.5 bg-slate-100 rounded-full overflow-hidden">
            <div className="h-full bg-primary rounded-full" style={{ width: `${executionCapacity}%` }} />
          </div>
          <p className="text-[11px] text-muted-foreground mt-3 font-medium">
                  ...
                  <div key={dec.id} className={cn("p-6 transition-all", isExpanded ? "bg-slate-50/50" : "hover:bg-slate-50/20")}>
                    ...
                    {isExpanded && (
                      <div className="mt-6 pt-6 border-t border-border grid grid-cols-1 md:grid-cols-2 gap-6 text-xs text-muted-foreground animate-slide-down">
                        ...
                        <div className="space-y-4 bg-slate-50 p-5 rounded-2xl border border-border">
                          <div>
                            <p className="font-bold text-muted-foreground flex items-center gap-1.5">
                              <Scale size={14} className="text-primary" /> Parâmetros de Prioridade
                            </p>
                            <div className="grid grid-cols-3 gap-3 mt-2.5">
                              <div className="bg-white p-2 rounded-xl border border-border text-center">
                                <span className="block text-[9px] font-semibold text-muted-foreground uppercase">Urgência</span>
                                <span className="font-extrabold text-muted-foreground">{dec.urgency}</span>
                              </div>
                              <div className="bg-white p-2 rounded-xl border border-border text-center">
                                <span className="block text-[9px] font-semibold text-muted-foreground uppercase">Impacto</span>
                                <span className="font-extrabold text-muted-foreground">{dec.impact}</span>
                              </div>
                              <div className="bg-white p-2 rounded-xl border border-border text-center">
                                <span className="block text-[9px] font-semibold text-muted-foreground uppercase">Sobrevivência</span>
                                <span className="font-extrabold text-muted-foreground">{dec.survivability}</span>
                              </div>
                              <div className="bg-white p-2 rounded-xl border border-border text-center">
                                <span className="block text-[9px] font-semibold text-muted-foreground uppercase">Redução Risco</span>
                                <span className="font-extrabold text-muted-foreground">{dec.riskReduction}</span>
                              </div>
                              <div className="bg-white p-2 rounded-xl border border-border text-center">
                                <span className="block text-[9px] font-semibold text-muted-foreground uppercase">Viabilidade</span>
                                <span className="font-extrabold text-muted-foreground">{dec.feasibility}</span>
                              </div>
                              <div className="bg-white p-2 rounded-xl border border-border text-center">
                                <span className="block text-[9px] font-semibold text-muted-foreground uppercase">DRS</span>
                                <span className="font-extrabold text-muted-foreground">{dec.dependencyReadiness}</span>
                              </div>
                            </div>
                          </div>
                          <div>
          ...
          <div className="bg-white border border-border rounded-3xl p-6 shadow-sm space-y-4">
            ...
            <div className="space-y-3">
              {capitalAllocationRanking.map((alloc: any) => (
                <div key={alloc.rank} className="flex items-center gap-3 p-3 bg-slate-50 border border-border rounded-2xl">
                  <div className="w-6 h-6 rounded-lg bg-slate-200 text-muted-foreground flex items-center justify-center font-bold text-xs">
                    {alloc.rank}
                  </div>
                  <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white border border-border rounded-3xl p-6 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-[10px] font-bold text-muted-foreground uppercase">Selecione uma Recomendação</label>
              <select 
                value={selectedSimDecisionId} 
                onChange={(e) => setSelectedSimDecisionId(e.target.value)}
                className="mt-1 block w-full bg-slate-50 border border-border rounded-xl p-2 text-xs font-bold text-muted-foreground focus:outline-none"
              >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {Object.entries(decisionScenarioMatrix[currentSimDecision.id]).map(([scName, scVal]: [string, any]) => (
                  <div key={scName} className="p-4 bg-slate-50 border border-border rounded-2xl space-y-2">
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{scName}</span>
                    <div className="grid grid-cols-2 gap-2 text-[10px] text-muted-foreground">
                      <div>
                        <strong className="block text-muted-foreground">Liquidez:</strong>
                        {scVal.liquidityImpact}
                      </div>
                      <div>
                        <strong className="block text-muted-foreground">Covenants:</strong>
                        {scVal.covenantImpact}
                      </div>
                      <div>
                        <strong className="block text-muted-foreground">Continuity:</strong>
                        {scVal.survivabilityImpact}
                      </div>
                      <div>
                        <strong className="block text-muted-foreground">Financiabilidade:</strong>
      ...
      <div className="bg-white border border-border rounded-3xl p-6 shadow-sm space-y-6">
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl w-max">
          <button
            ...
            className={cn("px-4 py-2 text-xs font-bold rounded-lg transition-all", selectedTimelineTab === 'immediate' ? "bg-white text-muted-foreground shadow-sm" : "text-muted-foreground hover:text-muted-foreground")}
          >
            Imediato (0-30 dias)
          </button>
          <button
            onClick={() => setSelectedTimelineTab('short')}
            className={cn("px-4 py-2 text-xs font-bold rounded-lg transition-all", selectedTimelineTab === 'short' ? "bg-white text-muted-foreground shadow-sm" : "text-muted-foreground hover:text-muted-foreground")}
          >
            Curto Prazo (30-90 dias)
          </button>
          <button
            onClick={() => setSelectedTimelineTab('medium')}
            className={cn("px-4 py-2 text-xs font-bold rounded-lg transition-all", selectedTimelineTab === 'medium' ? "bg-white text-muted-foreground shadow-sm" : "text-muted-foreground hover:text-muted-foreground")}
          >
            Médio Prazo (90-180 dias)
          </button>
          <button
            onClick={() => setSelectedTimelineTab('long')}
            className={cn("px-4 py-2 text-xs font-bold rounded-lg transition-all", selectedTimelineTab === 'long' ? "bg-white text-muted-foreground shadow-sm" : "text-muted-foreground hover:text-muted-foreground")}
          >
