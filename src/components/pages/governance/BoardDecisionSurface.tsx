      ...
      <div className="flex items-center gap-3 border-b border-border dark:border-white/5 pb-3 justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-900 hover:bg-slate-800 text-white text-[9px] font-mono font-bold uppercase tracking-widest rounded transition-colors border border-border shadow-sm"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {activeDirectives.length === 0 ? (
            <div className="p-6 rounded-2xl bg-slate-100/50 dark:bg-zinc-950/40 border border-border dark:border-zinc-900 text-center space-y-2 shadow-xs">
              <Shield className="w-8 h-8 text-muted-foreground dark:text-zinc-600 mx-auto" />
              <p className="text-xs text-muted-foreground dark:text-zinc-400 font-semibold uppercase tracking-wider">
                Nenhuma diretiva ativa no momento.
              </p>
              <p className="text-[10px] text-muted-foreground dark:text-zinc-500 max-w-sm mx-auto font-medium">
            ...
            <div className="space-y-3">
              {activeDirectives.map((dir, idx) => (
                <div key={idx} className="p-4 bg-slate-100/80 dark:bg-zinc-950/60 border border-border dark:border-zinc-900 rounded-2xl space-y-3 shadow-xs">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <span className="text-[10px] font-bold text-muted-foreground dark:text-zinc-300 font-mono">
                      {dir.id}
                    </span>
                    <div className="flex items-center gap-2">
                      <span className="text-[8px] font-mono bg-slate-200/60 dark:bg-zinc-900 text-muted-foreground dark:text-zinc-500 px-1.5 py-0.5 rounded border border-border dark:border-zinc-800 uppercase tracking-wider">
                  ...
                  {dir.causalDrivers && dir.causalDrivers.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 items-center">
                      <span className="text-[8px] font-black uppercase tracking-widest text-muted-foreground dark:text-zinc-650">
                        Gatilhos:
                      </span>
                      {dir.causalDrivers.map((driver, dIdx) => (
                        <span key={dIdx} className="text-[8px] font-mono bg-slate-200/50 dark:bg-zinc-900/60 text-muted-foreground dark:text-zinc-400 px-1.5 py-0.5 rounded border border-border dark:border-zinc-800">
                          {driver}
                        </span>
                      ))}
                    </div>
                    ...
                    {openCognitiveDrawer && (
                      <>
                        <button 
                          ...
                          className="flex items-center gap-1.5 px-2.5 py-1.5 bg-accent hover:bg-accent text-accent dark:text-accent border border-accent rounded text-[9px] font-black uppercase tracking-wider transition-colors"
                        >
                        ...
                        <button 
                          onClick={() => openCognitiveDrawer(dir.id)}
                          className="flex items-center gap-1.5 px-2.5 py-1.5 bg-slate-500/10 hover:bg-slate-500/20 text-muted-foreground dark:text-muted-foreground border border-border rounded text-[9px] font-black uppercase tracking-wider transition-colors"
                        >
                          <Search size={12} />
                          Ver Evidências
                        </button>
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 bg-slate-100/80 dark:bg-zinc-950/60 border border-border dark:border-zinc-900 rounded-2xl space-y-4 shadow-xs">
            <div className="flex justify-between items-center text-[10px] text-muted-foreground dark:text-zinc-500 border-b border-border dark:border-white/5 pb-2">
            ...
            {resolutions.length === 0 ? (
              ...
              <div className="space-y-3">
                {resolutions.map((resId, idx) => (
                  <div key={idx} className="p-3 bg-slate-200/30 dark:bg-zinc-900/40 border border-border dark:border-zinc-900 rounded-xl space-y-1.5 shadow-xs">
                    <div className="flex items-center justify-between text-[9px] font-mono">
                      <span className="font-bold text-muted-foreground dark:text-zinc-300">{resId}</span>
                      <div className="flex items-center gap-2">
                        <InvestigationLauncherWrapper 
                          tenantId="SYSTEM_TENANT" 
                          nodeId={resId} 
                        ...
                        {openCognitiveDrawer && (
                          <button 
                            ...
                            className="text-accent dark:text-accent hover:text-accent transition-colors"
                    ...
                    {approvals[idx] && (
                      <p className="text-[10px] text-muted-foreground dark:text-zinc-400 leading-snug font-medium italic">
                        "{approvals[idx]}"
                      </p>
                    )}
                  </div>
