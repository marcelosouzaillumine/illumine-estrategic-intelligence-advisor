...
export function ExecutiveAssessmentPage() {
  ...
  const calculateMaturity = () => {
    ...
    if (percentage < 0.65) return { level: 'Integrated Intelligence', desc: 'Existem conexões de dados, mas as decisões ainda dependem de consolidações lentas.', color: 'text-amber-500' };
    ...
    return { level: 'Institutional Intelligence', desc: 'Maturidade de ponta. A empresa possui Gêmeo Digital e memória causal para cada decisão.', color: 'text-amber-500' };
  };
            ...
            <div className="w-[90vw] md:w-[672px] mx-auto my-auto bg-white/5 border border-white/10 p-6 md:p-12 rounded-3xl relative z-10 text-center shadow-2xl backdrop-blur-sm animate-in fade-in slide-in-from-bottom-4 duration-700">
               ...
               <div className="text-left mb-8 bg-[#050506] border border-white/5 rounded-2xl p-6">
                  ...
                  <ul className="space-y-2 text-sm text-slate-400">
                     <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" /> Preservação contínua de contexto decisório.</li>
                     <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" /> Integração entre a estratégia aprovada e a execução tática.</li>
                     <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" /> Formação de uma memória institucional perene.</li>
                  </ul>
               </div>
            ...
            <div className="w-[90vw] md:w-[576px] mx-auto my-auto bg-white/5 border border-white/10 p-6 md:p-12 rounded-3xl relative z-10 shadow-2xl backdrop-blur-sm animate-in fade-in slide-in-from-right-8 duration-500">
               {formSubmitted ? (
                  <div className="text-center animate-in zoom-in-95 duration-500">
                     ...
                     <p className="text-slate-400 mb-8">Nossa equipe de concierges executivos entrará em contato em até 24h para agendar a sua apresentação privada da plataforma Illumine.</p>
                     <Link 
                        to="/"
                        className="w-full py-4 bg-transparent border border-white/10 text-slate-400 font-medium rounded-full hover:bg-white/5 transition-colors flex items-center justify-center"
                     >
                  ...
                  <>
                     <h3 className="text-2xl font-bold text-white mb-4">Solicitar Acesso ao Relatório Executivo</h3>
                     <div className="space-y-4 text-slate-400 mb-8">
                        <p>Sua organização está pronta para uma nova camada de inteligência institucional.</p>
                        <p>Preencha suas informações executivas para receber uma análise personalizada de maturidade organizacional e agendar uma apresentação privada da plataforma Illumine.</p>
                     </div>
                     ...
                     <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setFormSubmitted(true); }}>
                        <div>
                           ...
                           <input required type="text" className="w-full bg-[#050506] border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-primary transition-colors" placeholder="Ex: João Silva" />
                        </div>
                        <div>
                           <label className="block text-xs uppercase tracking-widest text-slate-500 mb-2 font-medium">Email Corporativo</label>
                           <input required type="email" className="w-full bg-[#050506] border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-primary transition-colors" placeholder="joao@empresa.com" />
                        </div>
                        <div>
