...
export function BoardPackPreviewModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal Container */}
      <div className="bg-[var(--color-background)] border border-white/10 w-full max-w-5xl max-h-[92vh] rounded-2xl shadow-2xl relative z-10 flex flex-col overflow-hidden text-white">
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden bg-slate-950/20">
          ...
          <div className="flex-1 p-6 md:p-8 flex flex-col items-center justify-center border-b lg:border-b-0 lg:border-r border-white/5 overflow-y-auto">
            ...
            <div className={`aspect-[16/9] w-full max-w-[720px] rounded-xl border border-white/10 shadow-2xl relative flex flex-col p-8 select-none transition-all duration-300 ${activeSlide.slideNumber === 1 || activeSlide.slideNumber === 12 ? 'bg-[var(--color-primary)] text-white' : 'bg-white text-muted-foreground'}`}>
              
              {/* Slide Header */}
              {activeSlide.slideNumber !== 1 && (
                <div className="flex justify-between items-start border-b border-border pb-2">
                  <div>
                    <h3 className={`text-base font-black tracking-tight ${activeSlide.slideNumber === 12 ? 'text-[var(--color-state-healthy)]' : 'text-[var(--color-primary)]'}`}>
                      {activeSlide.title}
                    </h3>
              ...
              <div className="flex-1 mt-6 flex flex-col justify-start overflow-y-auto">
                {activeSlide.slideNumber === 1 ? (
                  ...
                  <div className="h-full flex flex-col justify-between py-2">
                    <div className="w-8 h-8 bg-[var(--color-state-healthy)] rounded-md flex-shrink-0" />
                    <div>
                      <h1 className="text-2xl font-black tracking-tight text-white mb-2 text-primary">{activeSlide.title}</h1>
                      <p className="text-xs italic text-[var(--color-state-healthy)] font-light">"{pack.executiveHeadline}"</p>
                    </div>
                    
                    <div className="flex justify-between items-end border-t border-white/10 pt-4">
                      {/* Cognitive Badge */}
                      <div className="p-2 border border-[var(--color-state-healthy)]/30 bg-white/5 rounded-md">
                        <span className="text-[7px] font-bold text-[var(--color-state-healthy)] block uppercase tracking-widest">COGNITIVE CERTIFICATION</span>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {activeSlide.content.map((bullet, idx) => (
                      <div key={idx} className="p-2.5 bg-slate-50 border border-border rounded-lg flex items-center gap-2">
                        <span className="w-1 h-8 bg-[var(--color-state-healthy)] rounded flex-shrink-0" />
                    ...
                    {activeSlide.content.map((bullet, idx) => {
                      ...
                      return (
                        <div key={idx} className="p-2 bg-slate-50 border border-border rounded-lg flex items-center gap-2">
                          <span className={`w-1 h-6 rounded flex-shrink-0 ${isCritical ? 'bg-[var(--color-accent)]' : 'bg-[var(--color-state-healthy)]'}`} />
                          <p className="text-[9px] text-[var(--color-primary)] font-medium leading-normal">{bullet}</p>
                        </div>
                      );
                    })}
                  </div>
                    ...
                    {activeSlide.content.map((bullet, idx) => {
                      ...
                      return (
                        <div key={idx} className="p-2.5 bg-slate-50 border border-border rounded-lg flex items-start gap-2">
                          <span className="w-1 h-10 bg-[var(--color-accent)] rounded flex-shrink-0 mt-0.5" />
                          <div>
                            <p className="text-[10px] font-black text-[var(--color-primary)]">{parts[0]}</p>
                  ...
                  <ul className={`space-y-2.5 text-[10px] ${activeSlide.slideNumber === 12 ? 'font-mono text-muted-foreground' : 'text-muted-foreground'}`}>
                    {activeSlide.content.map((bullet, idx) => (
                      <li key={idx} className="flex items-start gap-2">
                        <span className={`w-1.5 h-1.5 rounded-full mt-1 flex-shrink-0 ${activeSlide.slideNumber === 12 ? 'bg-[var(--color-state-healthy)]' : 'bg-[var(--color-primary)]'}`} />
                ...
                return (
                  <button
                    ...
                    className={`w-full p-2.5 rounded-xl text-left text-xs transition-all flex items-center gap-2.5 border ${
                      isActive 
                        ? 'bg-[var(--color-accent)]/10 border-[var(--color-accent)]/40 text-[var(--color-accent)] font-bold shadow-md' 
                        : isAnnex 
                        ? 'bg-slate-950/40 border-white/5 hover:border-[var(--color-state-healthy)]/30 text-muted-foreground hover:text-[var(--color-state-healthy)]' 
                        : 'bg-slate-950/40 border-white/5 hover:border-white/10 text-muted-foreground hover:text-white'
                    }`}
                  >
                    <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-black ${
                      isActive ? 'bg-[var(--color-accent)] text-white' : isAnnex ? 'bg-[var(--color-state-healthy)]/20 text-[var(--color-state-healthy)]' : 'bg-white/5 text-muted-foreground'
                    }`}>
        ...
        <div className="p-5 border-t border-white/5 bg-slate-950/40 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center justify-end gap-3 flex-wrap">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-[var(--color-accent)] hover:bg-[var(--color-accent)]/90 rounded-xl text-xs font-bold uppercase tracking-widest text-white shadow-lg transition-all"
            >
              <Download size={14} />
