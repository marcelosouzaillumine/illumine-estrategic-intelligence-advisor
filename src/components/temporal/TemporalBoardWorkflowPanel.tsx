...
export const TemporalBoardWorkflowPanel: React.FC<TemporalBoardWorkflowPanelProps> = ({ 
  ...
  return (
    <div className="temporal-board-workflow p-6 bg-slate-900 border border-slate-700 rounded-lg shadow-md text-slate-100">
      <div className="mb-4 border-b border-slate-700 pb-2">
        <h3 className="text-lg font-semibold text-slate-50">Board Workflow State</h3>
        <p className="text-xs text-slate-400 mt-1">Official Escallation Control</p>
      </div>

      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="text-xs text-slate-400 mb-1">Current Protocol:</div>
        ...
        <div className="text-left md:text-right">
          <div className="text-xs font-mono text-slate-500">Audit Ref: {auditReference}</div>
          <div className="text-xs font-mono text-slate-500">Lineage: {lineageHash.substring(0, 8)}</div>
        </div>
      </div>
