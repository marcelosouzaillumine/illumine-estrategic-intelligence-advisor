...
export class ScenarioExplanationEngine {
  ...
  public static generateScenarioExplanation(
    ...
    explanation += `- contexto Candidato (${candidatePath.category}): Classificação de risco ${candidatePath.classification}. Score final composto de ${candidatePath.finalSurvivabilityScores?.composite}/100.\n`;
    ...
    explanation += `AVISO DE PROJEÇÃO: As trajetórias simuladas acima representam projeções de contextos limitadas (bounded scenario projections) com base em regras determinísticas e parâmetros históricos calibrados. `;
