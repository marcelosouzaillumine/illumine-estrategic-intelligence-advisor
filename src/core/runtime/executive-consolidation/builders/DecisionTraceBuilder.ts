...
export interface DecisionTraceNode {
  type: 'question' | 'opinion' | 'driver' | 'implication' | 'observation' | 'kpis' | 'engine' | 'source';
  label: string;
  content: string;
}
...
export class DecisionTraceBuilder {
  public static build(
    ...
    const action = BalanceSheetExecutiveOpinionBuilder.buildTechnicalObservation(scenario, facts);
    ...
    return [
      ...
      { type: 'observation', label: 'Observação Técnica', content: action },
