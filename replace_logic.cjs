...
content = content.replace(
  "import { collection, addDoc, updateDoc, deleteDoc, doc, serverTimestamp, query, where, getDocs, onSnapshot } from 'firebase/firestore';\n",
  ""
);
...
// 7. Replace approveClient
const oldApprove = `await updateDoc(doc(db, "clients", client.id), { approvalStatus: "Approved" });`;
const newApprove = `await ClientsApplicationService.approveClient(client.id);`;
