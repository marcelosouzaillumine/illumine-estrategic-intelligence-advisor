...
async function run() {
  console.log('Starting JS Identity Bridge Test...');

  const firebaseUid = 'test-firebase-uid-002';
  const canonicalUserId = '33333333-3333-4333-a333-333333333333'; 
  const tenantId = '33333333-3333-4333-a333-333333333333';
  ...
  const { error: userErr } = await adminClient.schema('tenant').from('users').upsert({
    id: canonicalUserId,
    email: 'test2@example.com',
    external_auth_id: firebaseUid,
    full_name: 'Identity Bridge Test User 2'
  });
  if (userErr) { console.error('User seed err:', userErr); process.exit(1); }

  const { error: tenantErr } = await adminClient.schema('tenant').from('tenants').upsert({
    id: tenantId,
    name: 'Identity Bridge Tenant 2',
    status: 'ACTIVE'
  });
  if (tenantErr) { console.error('Tenant seed err:', tenantErr); process.exit(1); }
  ...
  const { error: membershipErr } = await adminClient.schema('tenant').from('memberships').upsert({
  ...
  if (membershipErr) { console.error('Membership seed err:', membershipErr); process.exit(1); }

  console.log('Testing Identity Bridge Data Access...');

  const { data: extId, error: extErr } = await supabase.schema('tenant').rpc('get_my_external_id');
  console.log('Extracted External ID from JWT via PostgREST:', extId, extErr);
