...
// src/repositories/IdentityRepository.ts
...
var currentAccessTokenFetcher = null;
...
function registerSupabaseTokenFetcher(fetcher) {
  currentAccessTokenFetcher = fetcher;
}

// src/adapters/persistence/SupabaseIdentityAdapter.ts
var SupabaseIdentityAdapter = class {
  async getUserByAuthUid(uid) {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase.schema("tenant").from("users").select("*").eq("external_auth_id", uid).single();
    if (error || !data) return null;
    return this.mapToUser(data);
  }
  async getUserByEmail(email) {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase.schema("tenant").from("users").select("*").eq("email", email.toLowerCase().trim()).single();
    if (error || !data) return null;
    return this.mapToUser(data);
  }
  async createUser(user) {
    const supabase = getSupabaseClient();
    const { error } = await supabase.schema("tenant").from("users").insert({
      id: user.id,
      external_auth_id: user.authUid,
      email: user.email,
      full_name: user.email.split("@")[0],
      // fallback since domain User might not have fullName at this point
      status: user.status
    });
  ...
  async updateUserStatus(userId, status) {
    const supabase = getSupabaseClient();
    const { error } = await supabase.schema("tenant").from("users").update({ status }).eq("id", userId);
    if (error) throw new Error(error.message);
  }
...
// src/adapters/persistence/SupabaseTenantAdapter.ts
var SupabaseTenantAdapter = class {
  async getTenantById(tenantId) {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase.schema("tenant").from("tenants").select("*").eq("id", tenantId).single();
    if (error || !data) return null;
  ...
  async createTenant(tenant) {
    const supabase = getSupabaseClient();
    const { error } = await supabase.schema("tenant").from("tenants").insert({
      id: tenant.id,
      name: tenant.name,
  ...
  async updateTenant(tenantId, payload) {
    const supabase = getSupabaseClient();
    const { error } = await supabase.schema("tenant").from("tenants").update({
      name: payload.name,
  ...
  async deleteTenant(tenantId) {
    const supabase = getSupabaseClient();
    const { error } = await supabase.schema("tenant").from("tenants").delete().eq("id", tenantId);
    if (error) throw new Error(error.message);
  }
  async getMembershipsByUserId(userId) {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase.schema("tenant").from("memberships").select("*, roles(name)").eq("user_id", userId);
    if (error || !data) return [];
    return data.map(this.mapToMembership);
  }
  async getMembershipsByTenantId(tenantId) {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase.schema("tenant").from("memberships").select("*, roles(name)").eq("tenant_id", tenantId);
    if (error || !data) return [];
    return data.map(this.mapToMembership);
  }
  async createMembership(membership) {
  ...
  mapToTenant(row) {
    return {
      ...
      type: "CLIENT",
      ownerId: "SYSTEM",
      // Handled by RLS roles in Postgres
      createdAt: new Date(row.created_at)
    };
  }
  mapToMembership(row) {
...
var JWT_SECRET = "super-secret-jwt-token-with-at-least-32-characters-long";
async function run() {
  ...
  const forgedFirebaseJwt = import_jsonwebtoken.default.sign(
    {
    ...
    JWT_SECRET,
    { algorithm: "HS256", expiresIn: "1h", issuer: "https://securetoken.google.com/gen-lang-client-0196971385" }
  );
