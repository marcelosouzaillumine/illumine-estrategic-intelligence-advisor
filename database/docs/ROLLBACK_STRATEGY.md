...
2. The `legacy` schema acts as a buffer. If transformations fail, we drop `tenant`, `finance`, `governance`, and `audit`, leaving the extracted `legacy` intact to debug the SQL scripts.
