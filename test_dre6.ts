const dbData = [
  { category: 'Receita Operacional Bruta', val: 166513.28 },
  { category: '( - ) Deduções da Receita Bruta', val: 9543.74 },
  { category: '( = ) Receita Operacional Líquida', val: 156969.54 },
  { category: '( - ) Custo dos Produtos Vendidos', val: -70026.22 },
  { category: '( = ) Lucro Bruto', val: 86943.32 },
  { category: '( - ) Despesas Operacionais', val: -157385.83 },
  { category: 'Despesas Administrativas', val: -157385.83 },
];

const getOrderIndex = (name: string) => {
  const n = name.toLowerCase();
  // Match Totals and specific deductions BEFORE generic 'receita' or 'venda'
  ...
  if (n.includes('venda') && !n.includes('despesa') && !n.includes('imposto') && !n.includes('custo')) return 0; // Vendas as Receita Bruta
  ...
  return 5; // Default to Despesas Operacionais for unknowns
};
...
const matrix = [
  ...
  { name: '( - ) Despesas Operacionais', level: 1, type: 'parent', children: [] as any[], val: 0 },
];
...
const hasValue = (v: number) => Math.abs(v) > 0.001;

matrix[2].val = (hasValue(matrix[0].val) || hasValue(matrix[1].val)) ? (matrix[0].val + matrix[1].val) : matrix[2].val; // Receita Líquida
matrix[4].val = (hasValue(matrix[2].val) || hasValue(matrix[3].val)) ? (matrix[2].val + matrix[3].val) : matrix[4].val; // Lucro Bruto
