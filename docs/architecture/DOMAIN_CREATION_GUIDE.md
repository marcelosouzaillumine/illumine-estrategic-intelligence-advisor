...
Crie um novo diretório dentro de `src/governance/diagnostics/` com o nome do domínio.
Exemplo: `src/governance/diagnostics/operational/`
...
No arquivo `src/governance/diagnostics/core/diagnostic-types.ts`, adicione o seu novo tipo à `DiagnosticDomain`.
```typescript
...
Abra `src/governance/diagnostics/core/domain-registry.ts` e declare a existência do domínio em `registerCoreDomains()`.
```typescript
    this.register({
      domain: 'operational',
...
Abra `src/governance/diagnostics/catalog/register-diagnostics.ts` e exporte a classe para o Concierge.
```typescript
...
Vá em `src/governance/progression/executive-domain-graph.ts` e configure como esse domínio se conecta estrategicamente ao resto da organização. Adicione as influências dele e adicione quem o influencia.
```typescript
...
### Fim.
Seu novo domínio está ativo, será renderizado na cor "Future" no Architecture Map, orquestrado pela Narrativa se executado, e calculará pontos no Executive Governance Index automaticamente. Não toque no Core.
