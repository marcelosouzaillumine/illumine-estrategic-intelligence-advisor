# Architectural Freeze: v1.0 Constitutional Baseline
...
1. **Runtime Decides. UI Renders.** No fiduciary logic, threshold calculations, scoring, or narrative generation may occur in the presentation layer.
2. **Canonical Sovereignty:** The `CanonicalState` is the sole, untampered source of truth. UI components must render it verbatim.
3. **Fail-Closed Immutability:** If `CanonicalState` asserts a `CONSTITUTIONAL_QUARANTINE` or `FAIL_CLOSED` condition, the UI is strictly forbidden from displaying positive reinforcement or rendering restricted analytical sections.

## 3. Modification Protocols

### A. Structural Changes (RFC Protocol)
...
Future expansion into *ESG Runtime*, *Commercial Runtime*, *Operational Runtime*, or *People Runtime* MUST adhere to this v1.0 Baseline. Any new domain must act exclusively as an Engine, yielding to the Constitutional Governance Dashboard for final rendering authorization.
